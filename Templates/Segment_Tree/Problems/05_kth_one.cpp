...
struct Segtree {
   ...
   int kth_one(int x, int lx, int rx, int k) {
      if(tree[x].val <= k) return -1; // not here
      if(rx - lx == 1) {
         if(k > 0 || tree[x].val == 0) return -1;
         return lx;
      }
      int m = (rx + lx) >> 1;
      int id = kth_one(2 * x + 1, lx, m, k);
      if(id == -1) id = kth_one(2 * x + 2, m, rx, k - tree[2 * x + 1].val);
      return id;
   }

   int kth_one(int k) {
      return kth_one(0, 0, size, k);
   }
