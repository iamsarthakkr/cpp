...
struct Segtree {
   ...
   int destroy(int node, int lx, int rx, int l, int r, int p) {
      if(rx <= l || r <= lx) return 0;
      if(tree[node].mn > p) return 0;
      if(rx - lx == 1) {
         tree[node] = Info();
         return 1;
      }
      int m = (lx + rx) >> 1;
      int left = destroy(2 * node + 1, lx, m, l, r, p);
      int right = destroy(2 * node + 2, m, rx, l, r, p);
      recalc(node, lx, rx);

      return left + right;
   } 

   int destroy(int l, int r, int p) {
      return destroy(0, 0, size, l, r, p);
   } 
