...
struct Segtree {
   ...
   void apply(int l, int r, const Operation& op) {
      applyImpl(0, 0, size, l, r, op);
   };
...
private:
   ...
   void  applyImpl(int node, int lx, int rx, int l, int r, const Operation& op) {
      // disjoint
      if(rx <= l || r <= lx) {
         return;
      }
      // covered
      if(l <= lx && rx <= r) {
         tree[node].apply(lx, rx, op);
         return;
      }
      propagate(node, lx, rx);

      int m = (lx + rx) >> 1;
      applyImpl(2 * node + 1, lx, m, l, r, op);
      applyImpl(2 * node + 2, m, rx, l, r, op);
