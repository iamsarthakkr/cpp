...
public:
    template<typename T> T next(T min, T max) { return static_cast<T>(uniform_real_distribution<long double>(min, max)); } 
    template<typename T> T next(T max = numeric_limits<T>::max()) { return next(0, max); } 
